...
This file and `mf-operations-prototype.html` are the V3 originals, adopted
unchanged. Of everything in the V3 set they were the most current: the
prototype's `itemStatus()` and `packageState()` are the correct model, and
`scripts/status_engine.py` is a faithful port of them rather than a
replacement. `tests/test_status_engine.py` asserts the two agree on every
status, code, label, owner and action flag.

Where V3's other artifacts disagreed with this prototype — the Power Fx that
returned `0` for a not-due item, the three parallel status functions, the
colour-based rollup — **the prototype was right** and the rest is corrected.
See `handoffs/RECONCILIATION.md` C1 to C3.

Two things above are worth restating because they are easy to lose:

* The identity selector in the dark bar is **test harness only**. There is no
  sign-in, and it does not exist in the built app.
* "Upload is no longer a top-level destination" is about *navigation*, not
  about capability. The app is still the preferred front door and still
  declares installation, facility, requirement and period at upload — that is
  what keeps the stray queue small. What changed is that it sits inside the
  package flow rather than being a tab implying it is the only workflow.
  Folder drops keep working either way.
