...
Do not reorder this. **Do not put the app in front of users before EOM-01
produces correct rows** — every UI decision downstream depends on the shape of that data, and a
...
```powershell
python3 scripts/gen_rest_payloads.py   # regenerates provisioning/sharepoint-schema.json
...
one is missing. This is not a formality: past the 5,000-item List View
Threshold, adding an index to a large list is restricted and not to be
counted on, and `MF_EOM_Item` passes that inside the first year. Treat an
index missed here as unfixable-in-place — the realistic fix is a new list
and a migration — rather than betting on an exception later.
...
### 5. Assemble the canvas app — Path A, never by hand

**The app is not built screen-by-screen in Studio.** That was the pre-Path-A
sequence and it is retired: `CANVAS_APP_ASSEMBLY.md` is the current runbook.
Import Artifact 1, create ONE blank canvas app inside the solution, add the
19 data sources, bump the solution to 1.1.0, export, run
`scripts/assemble_full_solution.sh <export>.zip`, and import the assembled
candidate. Every screen, component and formula arrives from the validated
repository source through nine fail-closed gates. Manual screen-building
survives only as Path C, the no-CLI fallback, in that same runbook.

### 6. Verify `scrUpload` and EOM-02
...
### 7. Verify `scrInstallation` and `scrReview`
...
### 9. Verify EOM-02b and `scrUnmatched`
...
### 10. Enable EOM-04 with notifications disabled
...
### 12. Re-export, validate, promote

```bash
scripts/validate_final_export.sh <re-export>.zip
pac solution check --path <re-export>.zip   # DoD checker endpoint:
                                            # mil.api.advisor.appsplatform.us
sha256sum <re-export>.zip                   # append to SHA256SUMS.txt
```

The platform's re-export after open → publish is the canonical
Canvas-inclusive artifact (`CANVAS_APP_ASSEMBLY.md` steps 7–8 cover the
round-trip back into the repository, and the export-wins policy that
governs any diff). A maker-portal edit that never reaches the repository
is a change nobody can review or roll back — the round-trip is how it
reaches it.
...
2. **Connection references.** Supplied at import from the deployment settings
   file (`deployment/deployment-settings.example.json` is the template).
   **Bind all three, to the service account, at import** — the import
   checklist's rule governs. Notifications shipping off is enforced by the
   `NotificationsEnabled` flag and EOM-04 arriving disabled, never by
   leaving a connection unbound to fail at an unchosen moment later.
