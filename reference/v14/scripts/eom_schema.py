...
LISTS["MF_EOM_Submission"] = dict(
    ...
    columns=[
        ...
        c("SharePoint_File_ID", "Text",
          note="List item ID within the library. Survives a rename; the URL does not."),
        c("SharePoint_Unique_ID", "Text",
          note="The GUID. Survives a rename AND a move. This is the durable handle — resolve "
               "the URL from it rather than storing the URL as truth."),
        c("Destination_ID", "Text",
          note="FK to MF_Document_Destination — which configured destination received this file"),
        c("Source_Library", "Text", note="Library the file landed in"),
...
# ============================================================================
# v6 — document destinations
# The app never knows a path. It supplies Installation_ID, Reporting_Period and
# Requirement_ID; EOM-02 resolves the portfolio from the registry and builds the
# destination from this table. Moving Lackland to another portfolio is then an
# edit to MF_Installation.Portfolio_ID, not a change to the app.
# ============================================================================

LISTS["MF_Document_Destination"] = dict(
    title="MF Document Destination",
    grain="One row per portfolio per document domain",
    note="ADDED v6. Four portfolios, each with its own Monthly Documents folder. "
         "Standard, private and shared Teams channels have different SharePoint backing, "
         "so the site and library are configured per portfolio and never assumed. "
    ...
    columns=[
        ...
        c("Portfolio_ID", "Text", req=True, indexed=True),
        c("Document_Domain", "Choice", req=True, choices=["EOM", "EOY", "FMAT", "Other"],
          note="EOY shares the EOM destination unless a row says otherwise"),
        c("Site_URL", "Text", req=True,
          note="BLANK by default. Set at import from the environment variable. Never a "
               "literal in Power Fx. A portfolio backed by a private or shared channel has "
               "a different site than the parent team."),
        c("Library_Name", "Text", req=True, note="Usually Documents"),
        c("Root_Folder", "Text", req=True, note="Portfolio 2/Monthly Documents"),
        c("Folder_Template", "Text", req=True,
          note="{FiscalYear}/{ReportingPeriod}/{InstallationName}/{RequirementCode}. "
               "Tokens are resolved by EOM-02, never by the app."),
        c("Create_Missing_Folders", "Boolean", req=True,
          note="FALSE. The FY and month folders already exist and are curated by hand. The "
               "flow finds them; it does not create them. A flow that creates folders will "
               "eventually create 'Aug 26' beside someone's 'August 2026' and nobody notices "
               "for a month."),
        c("Fallback_Policy", "Choice", req=True,
          choices=["FIND_OR_ROOT", "FIND_OR_FAIL"],
          note="FIND_OR_ROOT: when the FY or month folder cannot be matched, drop the file at "
               "the Monthly Data Call root and flag it for a human to move. A submission that "
               "lands somewhere findable beats a submission that fails."),
        c("Site_Note", "Text",
          note="Which site this portfolio actually lives on. The four portfolios are FOUR "
               "SEPARATE SITE COLLECTIONS, not four folders in one library."),
