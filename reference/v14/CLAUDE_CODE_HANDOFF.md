...
```
Schema                     17 SharePoint lists, 249 columns
...
Tenant cloud               DoD (usaf.dps.mil / dod.teams.microsoft.us) — NOT GCC High
Portfolio sites            FOUR SEPARATE SITE COLLECTIONS, not four channels
Data-layer permissions     NOT ENFORCED — see the blocker below
Document destinations      4 portfolio rows seeded, all Site_URL blank,
                           all Channel_Type Unverified, all inactive
```
...
### 7. File routing — the app never knows a path

**Ruling: R1 uploads do not go to a generic intake library.** The submission
flow places evidence directly in its configured authoritative destination.

The app supplies `Installation_ID`, `Reporting_Period`, `Requirement_ID` and the
file. **EOM-02 resolves everything else.** It looks up the portfolio from
`MF_Installation`, finds the row in `MF_Document_Destination`, and builds the
path from a template:
...
This retires the central-intake-and-classify design for the normal path. That
existed because users dropped files in Teams and the system had to work out
afterward what they were. The app now knows before the file moves. `EOM-02b
Legacy Intake` keeps the discovery path for folder drops, as an exception route
only.

**Four portfolios, four Monthly Documents folders**, seeded in
`configuration/document-destinations.csv`. Every row ships with `Site_URL` blank,
`Channel_Type = Unverified` and `Active_Flag = FALSE`.

**Verify each portfolio channel's real SharePoint URL before enabling it.**
Standard, private and shared Teams channels have different SharePoint backing —
a private channel gets its own site, not a folder in the parent team's library.
Assuming four folders in one library is the most likely way this breaks on the
first real upload. EOM-02 fails closed on `DESTINATION_NOT_VERIFIED` rather than
guessing.

**Store the GUID, not the URL.** `SharePoint_Unique_ID` survives a rename and a
move; `File_URL` survives neither. The whole audit trail hangs off that pointer,
so resolve the URL from the GUID rather than treating the URL as truth.

**One authoritative copy.** One SharePoint file, one submission record pointing
at it. No central duplicate — that creates ambiguity about which copy is
authoritative, a retention problem, and broken links when the two diverge.

**Filenames stay as uploaded.** `Scan0023948.pdf` is fine. The record carries the
meaning. No convention is applied, required or inferred, and a resubmission lands
in the same requirement folder as v2 with v1 intact.
...
### 9. Design and the Figma prototype
...
```
ACTION_DOCUMENT.md              READ FIRST — the routing finding and current actions
CLAUDE_CODE_HANDOFF.md          this file — the accumulated build state
...
configuration/
  ...
  document-destinations.csv     4 SEPARATE SITES, root folders all differ
...
docs/
  ...
  native-visuals.md             in-app status visuals; why not the chart controls
...
flows/  EOM01 ExpectedPackage · EOM02 Submission · EOM02b LegacyIntake ·
        EOM03 Reconciliation · EOM04 Notifications   specs, not JSON
...
17. The app never knows a path. It supplies logical identifiers; EOM-02 resolves
    the destination.
18. Store `SharePoint_Unique_ID`. Resolve the URL from it. Never treat the URL
    as the durable handle.
19. One authoritative copy. No central intake duplicate.
20. Never write to an `Unverified` destination.
...
**Gate 3b — destinations**
- [ ] Open each of the four Portfolio channels and read its real SharePoint URL
- [ ] Record whether each is a Standard, Private or Shared channel
- [ ] Set `Site_URL`, `Library_Name`, `Root_Folder` per portfolio
- [ ] Set `Channel_Type` and record `Verified_By` / `Verified_Date`
- [ ] Set `Active_Flag = TRUE` only for verified rows
- [ ] Test one upload per portfolio and confirm the file lands where expected

**A private or shared channel has its own site, not a folder in the parent
team's library.** Assuming four folders in one library is the most likely way
this breaks on the first real upload.
...
7. **Are all four Portfolio channels standard?** If any is private or shared it
   has a different backing site, and the destination row needs its own
   `Site_URL`. Requires someone to open each channel and check.
8. **Does `Monthly Documents` already exist under each portfolio, and is that the
   exact folder name?** The seed assumes it. A mismatch is a one-cell edit if
   caught now and a failed pilot upload if not.
