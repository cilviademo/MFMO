...
**Read `docs/build-notes.md` FIRST.** It carries the programme's answers of
26 Aug 2026 and overrides earlier assumptions throughout this repo — including
the requirement matrix, the suspense rule and the entire security model.

Then `docs/status-calculation.md`, `docs/government-environment-mode.md`,
`docs/accessibility.md` and `docs/design-system.md`. Those three files contain the
...
```
Schema version:            4.0  (15 lists, 209 columns)
Requirement seed:          13 rows — 8 active. Authority and scope tracked
                           separately: 2 High / 3 Medium / 2 Proposed / 1 Low scope
Suspense:                  5 calendar days (VERIFIED) + 10th (MANAGEMENT_RULE)
                           Nominal and effective dates, NEXT_DUTY_DAY policy
Status palette:            SIX states — amber (time risk) and yellow (in review)
                           are distinct
Suspense rule:             CONFIRMED — 5 days after month end, final call the 10th
Security model:            CONFIRMED — CAC + GAL, installation-level, request path
Primary audience:          CONFIRMED — base users first, AFSVC as tracker
EOY:                       PARTIALLY DEFINED — MFR + inventory last page seeded;
                           grain, QC checklist, count-sheet retention still open
Food 2.0 / MAFFO:          DEFERRED — placeholders inactive
Registry:                  Critical R1 config dependency, NOT a blocker.
                           Generation_Enabled gates EOM-01 per installation.
...
| Fluent modern controls, not the v8 Teams theme hack | `docs/design-system.md` |
| Calendar as a canvas component, not PCF | `docs/design-system.md`, `MF_Calendar_Event` |
| Figma build is a design reference, NOT an import artifact | `docs/powerapps-translation.md` |
| Hover may refine, never reveal | `docs/powerapps-translation.md` |
| Period navigation, bulk accept, recall | `docs/design-system.md` |
