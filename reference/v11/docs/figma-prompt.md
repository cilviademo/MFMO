...
It is written to be pasted whole. Everything after it is context for you, not
for the tool.
...
## Notes for you, not for the tool

**Two prompts, if the tool struggles.** Generation tools tend to average
everything toward one look. If the launch screen contaminates the working
screens with navy, run the launch screen separately and keep the second prompt
starting at "2. HOME".

**The greyscale instruction is doing real work.** It is the fastest way to
catch a design that leans on colour to carry status, which is a 508 failure and
the single most likely one in this app.

**Expect to argue with it about radius.** Generation tools default to 8–16px
rounding and soft shadows because that is what most training data looks like.
If it comes back rounded and floaty, the fastest correction is: "Reduce all
corner radii to 2px, remove every box-shadow, and replace card separation with
1px borders."

**What deliberately is not in the prompt.** No mention of AI, agents, insights,
or anything that would pull it toward the marketing register of the Cognos
reference. That page is selling; this app is used. The visual language
transfers; the voice does not.

**After Figma, before Power Apps.** Export the tokens as a JSON and map them to
modern-theme values. Do not translate a Figma frame into absolute X/Y
coordinates in canvas — the layout above is built on containers and rules, and
it should stay that way. See `docs/accessibility.md`.
